...
sed -f ${OUTFILE}.temp.sed $F > ${F}.t.fastq
sed 's|^|/|;s|$|/,+3 d|' <(grep primer2 ${R}.t1.txt|awk -F"\t" '{print $1}') > ${OUTFILE}.temp.sed
sed -f ${OUTFILE}.temp.sed $F > ${R}.t.fastq 

grep adapter ${F}.t1.txt|awk -F"\t" '{print $1}'|sort|uniq|$SCRIPT_DIR/adapt_delete.pl ${F}.t.fastq > ${F}.t2.fastq
grep adapter ${R}.t1.txt|awk -F"\t" '{print $1}'|sort|uniq|$SCRIPT_DIR/adapt_delete.pl ${R}.t.fastq > ${R}.t2.fastq
...
rm ${F}.t1.txt ${R}.t1.txt ${F}.t.fastq ${R}.t.fastq ${F}.t2.fastq ${R}.t2.fastq ${OUTFILE}_t1.fa ${OUTFILE}_t2.fa ${OUTFILE}.temp.sed
