...
biom16 <- get_biom("Set170217/16S.taxa.biom","Set170217/colData")
biomITS <- get_biom("Set170217/ITS.taxa.biom","Set170217/colData")
biomOO <- get_biom("Set170217/OO.taxa.biom","Set170217/colData2")
biomNEM <- get_biom("Set170126/NEM.taxa.biom","Set170126/colData2")
...
res <- lapply(Ldds,function(obj) results(obj,alpha=alpha,parallel=T))
...
myfiltbioms <- lapply(mybioms,function(obj) prune_samples(sample_data(obj)$condition!="Control",obj))
...
lapply(seq(1,length(CHcoHmodels)), function(i) {
...
lapply(CFcoHmodels, function(obj) {
	nrow(obj$results[obj$results$padj<=0.1,]
