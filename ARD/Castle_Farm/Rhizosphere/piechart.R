...
g <- g + geom_label_repel(data=X[X$all<=4,],aes(y = pos), size=3, show.legend = F, nudge_x=0.75,nudge_y=0.75,segment.size=0.1)# + guides(fill = guide_legend(title = "rank"))
...
ggsave("test.pdf",g+xlab(NULL)+ylab(NULL))
