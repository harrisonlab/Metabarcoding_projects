...
cat ${OUTFILE}.t1.txt|awk -F"\t" '{print $1}'|sort|uniq|xargs -I ¬ sed -i -ne:t -e"/*\@¬.*/D" -e'$!N;//D;/'"\@¬/{" -e"s/\n/&/3;t" -e'$q;bt' -e\} -e's/\n/&/'"1;tP" -e'$!bt' -e:P  -e'P;D' ${OUTFILE}.t1
usearch8.1 -fastq_filter ${OUTFILE}.t1 -fastq_minlen $MINL -fastq_maxee_rate $QUAL -relabel $LABEL -fastaout $OUTFILE

mkdir -p $OUTDIR/../unfiltered 
mv ${OUTFILE}.t1 $OUTDIR/../unfiltered/${OUTFILE}.unfiltered.fastq
